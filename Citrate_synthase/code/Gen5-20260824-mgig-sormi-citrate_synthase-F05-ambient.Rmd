---
title: "Gen5-20260824-mgig-sormi-citrate_synthase-F05-ambient"
author: "Sam White"
date: "2026-08-24"
output:
  github_document:
...
oyster) individuals from **family F05** held at **ambient** temperature,
re-assayed 2026-08-24 with the
...
2026-08-11 (`Gen5-20260811-mgig-citrate_synthase-F05-ambient.Rmd`), re-run
with a shorter read window and a different background-control design (see
note 3).
...
`F05_08_ambient`, re-assayed at dilution). These are the same homogenates
and the same protein measurements used for the original 2026-08-11 assay of
this plate.
...
for sample wells (e.g. `F05_01_ambient-citrate_synthase-10.1-df.0`),
`STD-<assay_type>-<nmol_per_well>` for GSH standards, `POS-<assay_type>` for
the positive control, and `BG-<assay_type>` for the pooled background
control (see note 3) -- the background label carries no weight or dilution
suffix, since it is not tied to any single individual's tissue.
The trailing number in the sample label (e.g. `10.1`) is tissue weight in mg,
...
1. **Read duration for this plate is 20 minutes** (11 reads at 2-min
   ...
   `read_duration_min = 20` to match. Rate extraction uses a fixed-width
   sliding window (`rate_window_n = 5` points = 8 min) rather than the full
   trace, so this only changes how many windows are available to slide
   across; see the KINETIC TRACES section for whether the assay is still in
   its linear phase over this window.
2. **Activity is normalized to total extracted protein, not tissue weight**,
   ...
   efficiency in a way tissue weight cannot. These are the same protein
   measurements used for the original 2026-08-11 ambient plate (same
   homogenates), and no `Sample` string in either protein-concentration CSV
   needed correction for this plate.
3. **Background correction uses a single pooled background control, not a
   paired per-sample background as on the 2026-08-11 and 2026-08-14 plates.**
   A pool was made from 5 &micro;L of each of the 8 sample homogenates, and
   ...
   Substrate Mix) to produce wells `BG-citrate_synthase` (D10-D12). Because
   there is only one background estimate for the whole plate, the same
   background-corrected rate is subtracted from every sample, rather than
   each sample carrying its own paired background replicate set.
...
This plate's `absorbance-*.csv` covers the full 48 occupied wells (24 sample
...
BCA/Bradford-style plate (595 nm) and supplied as two CSV exports -- the same
measurements used for the original 2026-08-11 assay of these homogenates.
...
Every sample and positive-control trace on this plate rises across the full
20-minute window (replicate-level R<sup>2</sup> > 0.99 for the fitted rate
window on every sample well, computed below). The three pooled-background
traces (D10-D12) are visibly near-flat and far lower in absolute A412 than
any sample well, consistent with a well-behaved background control. This
plate shows no elevated-starting-absorbance or contamination signature in
any sample well.
...
Every flagged well on this plate is a **GSH standard**, and every flag is
`flag_decreasing` -- 15 of the 18 standard wells (all five non-zero
concentrations: 8, 16, 24, 32, 40 nmol) end at or below their own starting
A412 over the 20-minute window, consistent with TNB<sup>2-</sup> signal
decay (see GSH STANDARD CURVE below). This is a substantially larger
fraction than the 20260814 plate (3/18 standards lost signal there), but
because the standard curve is calibrated at t = 0 (not from the endpoint),
it does not compromise the calibration (R<sup>2</sup> = 0.9983, no outlier
wells). **No sample well, positive-control well, or background well is
flagged for anything on this plate** -- no read glitches, no elevated
baseline, and no background well behaving as an active reaction.
...
All three pooled-background replicates (`D10`, `D11`, `D12`) are flat and
well-behaved: starting A412 0.055-0.064 (well below the 0.15 threshold) and
net change 0.002-0.003 over the full 20 minutes (well below the 0.05 drift
threshold). All three are used in the background-rate estimate below.
...
— sample wells all start within a tight 0.118-0.136 range and the pooled
background wells start even lower (0.055-0.064), both well below the 0.35
threshold. No sample has any elevated-baseline replicate, and none is
...
concentration's triplicate CV is under 2.1% (highest: 8 nmol at 2.1%), and
no well deviates from its triplicate median by more than the 0.15 A412
threshold. Because there are no outliers to exclude, the "outlier-excluded"
and "all wells" fits are identical: slope = `r sprintf("%.5f", std_slope)`
A412/nmol, R<sup>2</sup> = `r sprintf("%.4f", std_r2)`.
**Signal decay is more widespread than on prior plates**, however: 13 of 15
non-zero-concentration standards lose signal strictly (drift < 0) between
t = 0 and t = 20 min (the remaining 2 are unchanged to three decimals), and
15 of 18 are flagged `flag_decreasing` above once ties at exactly zero net
change are included. Because the curve is read at t = 0, this decay affects
none of the values used for calibration.
...
range here -- the highest reading is A412 1.43, below the 1.5 ceiling -- so
...
mA412/min, a considerably wider spread than the raw rates seen on this same
family's 36 &deg;C plate, reflecting real inter-individual variation among
these ambient-temperature homogenates (see CITRATE SYNTHASE ACTIVITY below).
...
0.997-1.000), confirming that the core reaction chemistry and reader were
functioning. Its **replicate-to-replicate precision is notably worse than on
prior plates**, though: `D7` rose at only 2.00 mA412/min versus 2.95 and
3.15 mA412/min for `D8`/`D9`, a 22.8% CV across the triplicate (well above
the 15% technical-CV threshold applied to samples). Because the positive
control is a validity check on assay chemistry, not an input to any
sample's activity calculation, this does not affect the results table below
-- but it is flagged again in the SUMMARY as a caution for future plates.

# BACKGROUND CORRECTION

Unlike the 2026-08-11 and 2026-08-14 plates, which paired each sample with
its own triplicate background control, this plate uses a **single pooled
background control** (note 3): the background rate is estimated once, from
the flat (well-behaved) replicates of the `BG-citrate_synthase` triplicate,
and the same corrected rate is subtracted from every sample.
...
than the paired per-sample backgrounds on the 36 &deg;C plate (~7%) -- a
consequence of the pooled background containing only 2 &micro;L of a
1-in-8 pooled homogenate per well, rather than the full undiluted 2
&micro;L of a single sample's own homogenate.
...
`F05_01_ambient` at 4.6%; the lowest is `F05_08_ambient` at 0.8%, and every
sample falls in the 0.8-4.6% range -- tighter than the positive control's
own 22.8% CV noted above.
...
reflects genuine plate-to-plate biological/extraction variation rather than
measurement noise.
...
This plate tells a **different story from the 20260814 (36 &deg;C) plate**:
...
all 8 samples clean on this plate). Because raw signal genuinely increases
with more protein loaded -- consistent with more CS enzyme being present
when more protein was extracted -- **the negative mU/mg protein trend here
is more consistent with protein normalization doing its intended job**
(correcting for real inter-sample extraction-efficiency differences) than
with a pure division artifact. Protein normalization also compresses the
inter-individual spread, from a 2.69-fold raw range down to a 1.64-fold
range in mU/mg protein, the opposite direction from the 36 &deg;C plate
(where normalizing widened, rather than narrowed, the apparent spread).
...
1. **No sample well, positive-control well, or background well is flagged
   for anything on this plate.** No decreasing kinetics, no read glitches,
   no elevated starting absorbance, and no background well behaving as an
   active reaction (all three pooled-background replicates are flat).
2. **Widespread but harmless GSH standard signal decay.** 13 of 15 (strict
   drift < 0) to 15 of 18 (including exact-zero ties) non-zero-concentration
   standard wells lose signal between t = 0 and t = 20 min, consistent with
   TNB<sup>2-</sup> instability. This is a larger fraction than on prior
   plates, but the standard curve is calibrated at t = 0, so it has no
   effect on the fit (R<sup>2</sup> = 0.9983, zero flagged outlier wells).
3. **Elevated positive-control replicate CV.** The three positive-control
   replicates rise linearly (R<sup>2</sup> 0.997-1.000) but disagree in rate
   by 22.8% CV (`D7` at 2.00 mA412/min vs. `D8`/`D9` at 2.95-3.15
   mA412/min), worse precision than any individual sample's technical
   triplicate on this plate. This does not affect sample activity values
   (the positive control is a chemistry validity check only), but is worth
   watching on future plates.
...
replicate is excluded from any sample's rate calculation on this plate, so
the "all reps" and "usable reps only" CVs are identical throughout.

## Assay validity

The **positive control worked** in the sense that matters for calibration:
all three replicates rise linearly (R<sup>2</sup> > 0.997), confirming the
core reaction chemistry and reader were functioning. Its elevated
replicate-to-replicate CV (22.8%, note above) is a caution rather than a
disqualifier, since it does not enter the activity calculation. The GSH
standard curve has zero flagged outlier wells and every sample well's
kinetics are clean, so this plate's activity values carry low calibration
uncertainty.
...
2. **Protein normalization looks appropriate for this plate, not
   ...
   with total extracted protein
   (r = `r sprintf("%+.2f", uL_trend_clean$estimate)`, p =
   `r sprintf("%.4f", uL_trend_clean$p.value)`), the opposite pattern from
   the 20260814 (36 &deg;C) plate, so the negative mU/mg-protein trend here
   is better explained by genuine extraction-efficiency differences between
   samples than by a division artifact. mU/mg protein is therefore a
   reasonable basis for comparison on this plate.
3. **Watch the positive control's replicate precision on future plates.**
   A 22.8% CV across a positive-control triplicate is unusually high; if it
   recurs, it is worth checking pipetting consistency for that specific
   reagent (e.g., undissolved/settled positive control stock, or a
   multichannel-pipetting order effect).
4. **The pooled-background design cuts the well count needed for
   ...
   estimate; on this plate
   the background rate is a small fraction of the sample signal (~2.2%)
   either way, so the simplification appears low-risk here, but a
   pooled design should be re-validated if background ever becomes a
   larger fraction of the raw signal.

## What can be used from this plate

All eight samples are usable, and unlike the 36 &deg;C plate, this ambient
plate shows real, well-resolved inter-individual variation in CS activity:
background-corrected rates range `r sprintf("%.2f", min(cs_activity$corrected_mOD_min))`
to `r sprintf("%.2f", max(cs_activity$corrected_mOD_min))` mA412/min across
the eight `F05` individuals at ambient temperature, a
`r sprintf("%.0f", 100 * (max(cs_activity$corrected_mOD_min) / min(cs_activity$corrected_mOD_min) - 1))`%
spread against 0.8-4.6% technical CV per sample -- far larger than the
technical noise floor, and in the opposite direction from the essentially
uniform activity seen across this same family at 36 &deg;C.

- **Protein-normalized activity (mU/mg protein) is the recommended basis
  for downstream comparison on this plate**, since raw activity correlates
  strongly with total extracted protein (see note above) -- normalizing is
  removing a real extraction-efficiency covariate here, not introducing an
  artifact.
- **The elevated positive-control CV (22.8%) is a plate-level caution**, not
  a sample-level one: it does not touch any individual sample's activity
  value, but should inform how much weight this plate's absolute activity
  values carry relative to future re-assays with tighter positive-control
  precision.
