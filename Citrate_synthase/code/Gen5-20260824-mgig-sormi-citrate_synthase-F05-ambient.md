...
temperature, re-assayed 2026-08-24 with the [Abcam Citrate Synthase
Assay Kit (ab239712),
...
This is the same eight `F05` ambient homogenates originally assayed
2026-08-11 (`Gen5-20260811-mgig-citrate_synthase-F05-ambient.Rmd`),
re-run with a shorter read window and a different background-control
design (see note 3).
...
(`F05_04_ambient`, `F05_05_ambient`, `F05_07_ambient`) and
...
(`F05_01_ambient`, `F05_02_ambient`, `F05_03_ambient`, `F05_06_ambient`,
`F05_08_ambient`, re-assayed at dilution). These are the same
homogenates and the same protein measurements used for the original
2026-08-11 assay of this plate.
...
sample wells (e.g. `F05_01_ambient-citrate_synthase-10.1-df.0`),
...
tissue. The trailing number in the sample label (e.g. `10.1`) is tissue
weight in mg, which is **not** used for normalization here (note 2); it
is parsed and validated but plays no role in the activity calculation.

## 1.2 Notes

1.  **Read duration for this plate is 20 minutes** (11 reads at 2-min
    intervals) – shorter than the original 2026-08-11 run of these same
    homogenates (40 min, 21 reads). The assay parameters below set
    `read_duration_min = 20` to match. Rate extraction uses a
    fixed-width sliding window (`rate_window_n = 5` points = 8 min)
    rather than the full trace, so this only changes how many windows
    are available to slide across; see the KINETIC TRACES section for
    whether the assay is still in its linear phase over this window.
2.  **Activity is normalized to total extracted protein, not tissue
    ...
    These are the same protein measurements used for the original
    2026-08-11 ambient plate (same homogenates), and no `Sample` string
    in either protein-concentration CSV needed correction for this
    plate.
3.  **Background correction uses a single pooled background control, not
    a paired per-sample background as on the 2026-08-11 and 2026-08-14
    plates.** A pool was made from 5 µL of each of the 8 sample
    homogenates, and 2 µL of that pool (matching `sample_volume_uL` used
    for every reaction well) was run in triplicate with Background
    Control Mix (no CS Substrate Mix) to produce wells
    `BG-citrate_synthase` (D10-D12). Because there is only one
    background estimate for the whole plate, the same
    background-corrected rate is subtracted from every sample, rather
    than each sample carrying its own paired background replicate set.
...
```
    ...
    absorbance CSV : 48 wells x 11 timepoints
...
This plate’s `absorbance-*.csv` covers the full 48 occupied wells (24
...
exports – the same measurements used for the original 2026-08-11 assay
of these homogenates. No `Sample` string typo needed correction for this
plate’s samples.
...
![](Gen5-20260824-mgig-sormi-citrate_synthase-F05-ambient_files/figure-gfm/plot-kinetic-traces-1.png)<!-- -->
...
Every sample and positive-control trace on this plate rises across the
full 20-minute window (replicate-level R<sup>2</sup> \> 0.99 for the
fitted rate window on every sample well, computed below). The three
pooled-background traces (D10-D12) are visibly near-flat and far lower
in absolute A412 than any sample well, consistent with a well-behaved
background control. This plate shows no elevated-starting-absorbance or
contamination signature in any sample well.
...
```
    ...
    Flag counts across all 48 wells:
...
Every flagged well on this plate is a **GSH standard**, and every flag
is `flag_decreasing` – 15 of the 18 standard wells (all five non-zero
concentrations: 8, 16, 24, 32, 40 nmol) end at or below their own
starting A412 over the 20-minute window, consistent with
TNB<sup>2-</sup> signal decay (see GSH STANDARD CURVE below). This is a
substantially larger fraction than the 20260814 plate (3/18 standards
lost signal there), but because the standard curve is calibrated at t =
0 (not from the endpoint), it does not compromise the calibration
(R<sup>2</sup> = 0.9983, no outlier wells). **No sample well,
positive-control well, or background well is flagged for anything on
this plate** – no read glitches, no elevated baseline, and no background
well behaving as an active reaction.
...
All three pooled-background replicates (`D10`, `D11`, `D12`) are flat
and well-behaved: starting A412 0.055-0.064 (well below the 0.15
threshold) and net change 0.002-0.003 over the full 20 minutes (well
below the 0.05 drift threshold). All three are used in the
background-rate estimate below.
...
```
    ...
    Sample/background wells with elevated starting A412 (> 0.35):0/27
...
0.35 — sample wells all start within a tight 0.118-0.136 range and the
pooled background wells start even lower (0.055-0.064), both well below
the 0.35 threshold. No sample has any elevated-baseline replicate, and
none is disqualified on this basis.
...
![](Gen5-20260824-mgig-sormi-citrate_synthase-F05-ambient_files/figure-gfm/plot-baseline-1.png)<!-- -->
...
: Standard wells flagged as outliers (\|deviation\| \> 0.15 A412)
    ...
    'data.frame':   3 obs. of  5 variables:
     ...
     $ n        : int  18 6 18
     $ slope    : num  0.0323 0.0323 0.0323
     $ intercept: num  0.107 0.107 0.107
     $ r_squared: num  0.998 0.999 0.998
...
concentration’s triplicate CV is under 2.1% (highest: 8 nmol at 2.1%),
and no well deviates from its triplicate median by more than the 0.15
A412 threshold. Because there are no outliers to exclude, the
“outlier-excluded” and “all wells” fits are identical: slope = 0.03234
A412/nmol, R<sup>2</sup> = 0.9983. **Signal decay is more widespread
than on prior plates**, however: 13 of 15 non-zero-concentration
standards lose signal strictly (drift \< 0) between t = 0 and t = 20 min
(the remaining 2 are unchanged to three decimals), and 15 of 18 are
flagged `flag_decreasing` above once ties at exactly zero net change are
included. Because the curve is read at t = 0, this decay affects none of
...
![](Gen5-20260824-mgig-sormi-citrate_synthase-F05-ambient_files/figure-gfm/plot-standard-curve-1.png)<!-- -->
...
linear range here – the highest reading is A412 1.43, below the 1.5
...
positive-net-change usability bar; every replicate-level R<sup>2</sup>
exceeds 0.998 (median 0.9997). No well needed to be excluded from rate
extraction on this plate – a direct consequence of the clean,
monotonically-rising traces noted above. Sample rates range 5.50-14.80
mA412/min, a considerably wider spread than the raw rates seen on this
same family’s 36 °C plate, reflecting real inter-individual variation
among these ambient-temperature homogenates (see CITRATE SYNTHASE
ACTIVITY below).
...
(R<sup>2</sup> 0.997-1.000), confirming that the core reaction chemistry
and reader were functioning. Its **replicate-to-replicate precision is
notably worse than on prior plates**, though: `D7` rose at only 2.00
mA412/min versus 2.95 and 3.15 mA412/min for `D8`/`D9`, a 22.8% CV
across the triplicate (well above the 15% technical-CV threshold applied
to samples). Because the positive control is a validity check on assay
chemistry, not an input to any sample’s activity calculation, this does
not affect the results table below – but it is flagged again in the
SUMMARY as a caution for future plates.

# 7 BACKGROUND CORRECTION

Unlike the 2026-08-11 and 2026-08-14 plates, which paired each sample
with its own triplicate background control, this plate uses a **single
pooled background control** (note 3): the background rate is estimated
once, from the flat (well-behaved) replicates of the
`BG-citrate_synthase` triplicate, and the same corrected rate is
...
All three background replicates are flat and are used in the estimate
(0.233 mA412/min), a much smaller fraction of the sample signal (~2.2%)
than the paired per-sample backgrounds on the 36 °C plate (~7%) – a
consequence of the pooled background containing only 2 µL of a 1-in-8
pooled homogenate per well, rather than the full undiluted 2 µL of a
single sample’s own homogenate.
...
`F05_01_ambient` at 4.6%; the lowest is `F05_08_ambient` at 0.8%, and
every sample falls in the 0.8-4.6% range – tighter than the positive
control’s own 22.8% CV noted above.
...
![](Gen5-20260824-mgig-sormi-citrate_synthase-F05-ambient_files/figure-gfm/plot-replicate-cv-1.png)<!-- -->
...
2.  **Background-corrected rate** = sample rate - the single pooled
    background rate (note 3), applied identically to every sample
3.  **B/dT** (nmol/min) = background-corrected rate / standard curve
    slope (0.03234 A412 per nmol, from the outlier-excluded fit)
...
![](Gen5-20260824-mgig-sormi-citrate_synthase-F05-ambient_files/figure-gfm/plot-activity-1.png)<!-- -->
...
Raw activity (mU/µL) spans a real 2.7-fold range across the eight
individuals – 0.0832 (`F05_05_ambient`) to 0.2242 (`F05_08_ambient`) –
against a technical CV of under 5% for any single sample, so this spread
reflects genuine plate-to-plate biological/extraction variation rather
...
![](Gen5-20260824-mgig-sormi-citrate_synthase-F05-ambient_files/figure-gfm/plot-activity-normalized-1.png)<!-- -->
...
This plate tells a **different story from the 20260814 (36 °C) plate**:
raw activity (mU/µL) rises strongly and significantly with total
extracted protein (r = +0.956, p = 0.0002), while mU/mg protein
correlates negatively with total extracted protein (r = -0.888, p =
0.0032, n = 8, all 8 samples clean on this plate). Because raw signal
...
Protein normalization also compresses the inter-individual spread, from
a 2.69-fold raw range down to a 1.64-fold range in mU/mg protein, the
opposite direction from the 36 °C plate (where normalizing widened,
rather than narrowed, the apparent spread).
...
1.  **No sample well, positive-control well, or background well is
    flagged for anything on this plate.** No decreasing kinetics, no
    read glitches, no elevated starting absorbance, and no background
    well behaving as an active reaction (all three pooled-background
    replicates are flat).
2.  **Widespread but harmless GSH standard signal decay.** 13 of 15
    (strict drift \< 0) to 15 of 18 (including exact-zero ties)
    non-zero-concentration standard wells lose signal between t = 0 and
    t = 20 min, consistent with TNB<sup>2-</sup> instability. This is a
    larger fraction than on prior plates, but the standard curve is
    calibrated at t = 0, so it has no effect on the fit (R<sup>2</sup> =
    0.9983, zero flagged outlier wells).
3.  **Elevated positive-control replicate CV.** The three
    positive-control replicates rise linearly (R<sup>2</sup>
    0.997-1.000) but disagree in rate by 22.8% CV (`D7` at 2.00
    mA412/min vs. `D8`/`D9` at 2.95-3.15 mA412/min), worse precision
    than any individual sample’s technical triplicate on this plate.
    This does not affect sample activity values (the positive control is
    a chemistry validity check only), but is worth watching on future
    plates.

## 11.2 Technical replicate precision

**All eight samples pass the 15% CV threshold** — the highest is
`F05_01_ambient` at 4.6%, and the lowest is `F05_08_ambient` at 0.8%
(`F05_02_ambient` 4.1%, `F05_03_ambient` 2.2%, `F05_04_ambient` 2.5%,
`F05_05_ambient` 1.9%, `F05_06_ambient` 1.9%, `F05_07_ambient` 1.3%). No
replicate is excluded from any sample’s rate calculation on this plate,
so the “all reps” and “usable reps only” CVs are identical throughout.

## 11.3 Assay validity

The **positive control worked** in the sense that matters for
calibration: all three replicates rise linearly (R<sup>2</sup> \>
0.997), confirming the core reaction chemistry and reader were
functioning. Its elevated replicate-to-replicate CV (22.8%, note above)
is a caution rather than a disqualifier, since it does not enter the
activity calculation. The GSH standard curve has zero flagged outlier
wells and every sample well’s kinetics are clean, so this plate’s
activity values carry low calibration uncertainty.
...
1.  **This plate does not need re-assay.** No sample is disqualified, no
    replicate is excluded, and the standard curve has no outlier wells.
2.  **Protein normalization looks appropriate for this plate, not
    artifactual.** Raw rate (mU/µL) is strongly and significantly
    correlated with total extracted protein (r = +0.96, p = 0.0002), the
    opposite pattern from the 20260814 (36 °C) plate, so the negative
    mU/mg-protein trend here is better explained by genuine
    extraction-efficiency differences between samples than by a division
    artifact. mU/mg protein is therefore a reasonable basis for
    comparison on this plate.
3.  **Watch the positive control’s replicate precision on future
    plates.** A 22.8% CV across a positive-control triplicate is
    unusually high; if it recurs, it is worth checking pipetting
    consistency for that specific reagent (e.g., undissolved/settled
    positive control stock, or a multichannel-pipetting order effect).
4.  **The pooled-background design cuts the well count needed for
    background control roughly 8-fold** (3 wells vs. 24 on the paired
    per-sample design) at the cost of losing any per-sample background
    estimate; on this plate the background rate is a small fraction of
    the sample signal (~2.2%) either way, so the simplification appears
    low-risk here, but a pooled design should be re-validated if
    background ever becomes a larger fraction of the raw signal.

## 11.5 What can be used from this plate

All eight samples are usable, and unlike the 36 °C plate, this ambient
plate shows real, well-resolved inter-individual variation in CS
activity: background-corrected rates range 5.38 to 14.50 mA412/min
across the eight `F05` individuals at ambient temperature, a 169% spread
against 0.8-4.6% technical CV per sample – far larger than the technical
noise floor, and in the opposite direction from the essentially uniform
activity seen across this same family at 36 °C.

- **Protein-normalized activity (mU/mg protein) is the recommended basis
  for downstream comparison on this plate**, since raw activity
  correlates strongly with total extracted protein (see note above) –
  normalizing is removing a real extraction-efficiency covariate here,
  not introducing an artifact.
- **The elevated positive-control CV (22.8%) is a plate-level caution**,
  not a sample-level one: it does not touch any individual sample’s
  activity value, but should inform how much weight this plate’s
  absolute activity values carry relative to future re-assays with
  tighter positive-control precision.
