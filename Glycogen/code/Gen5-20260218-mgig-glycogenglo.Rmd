...
# Extract concentration values from plate layout (row 6, columns 1-5)
# Parse concentration from labels like "STD-glyc-20" -> 20
glyc_concentrations <- as.numeric(gsub("STD-glyc-", "", plate_layout2[6, 1:5]))
glyc_concentrations

# Extract standard curve luminescence values for each replicate from raw_luminescence
glyc_row_F <- as.numeric(raw_luminescence2[6, 1:5])  # Row 6 (F)
glyc_row_G <- as.numeric(raw_luminescence2[7, 1:5])  # Row 7 (G)
glyc_row_H <- as.numeric(raw_luminescence2[8, 1:5])  # Row 8 (H)
```
```{r glycogen sample, include=TRUE}
#Extract glycogen sample data - wells D2-E6
glyc_sample_cols1 <- c(1,2,3)
...
Sample E5 is out of the standard curve range. This sample was re-run with further dilutions on 20260330
