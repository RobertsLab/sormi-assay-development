  ...
  if (hasPrecomputedAuc) {
    ...
    for (const d of plottedSorted) {
      ...
      if (!byTrace.has(d.trace_id)) {
        ...
        byTrace.get(d.trace_id).n_timepoints += 1
      }
    }
...
// Dots are now centered exactly on the series group
      ...
      statDesc.innerHTML = `<strong>Tests computed:</strong> Kruskal-Wallis (omnibus test across all groups), and pairwise Wilcoxon rank-sum tests with Benjamini–Hochberg p-value adjustment (blanks excluded). The summary below answers whether the groups differ overall, and which group pairs differ after adjustment.`
      ...
      if (relevantStats.length === 0) {
        ...
        summary.innerHTML = `<strong>${kwText}</strong><br>${sigPairText}<br><span style="color:#6b7280">These tests treat wells as replicates within the selected group column.</span>`
