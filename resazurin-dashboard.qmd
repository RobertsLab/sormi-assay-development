...
validGroupDisplaySel = rawGroupDisplaySel.filter(v => v === "All" || groupValues.includes(v))
...
filtered = selectionRows.filter(d =>
  ...
  (activeGroupCol === null ||
    (boxPlotMode
      ? allGroupsDisplayed || selectedDisplayGroups.includes(`${d[activeGroupCol]}`.trim())
      : activeGroupVal === "All" || `${d[activeGroupCol]}` === `${activeGroupVal}`))
)
...
seriesKey = d => {
  ...
  if (activeAggregationMode === "Individual replicates" && activeGroupCol !== null) {
...
plottedEffective = (() => {
  ...
  const showIndiv = activeAggregationMode === "Individual replicates" && activeGroupCol !== null
