...
aucData = (() => {
  // Prefer precomputed `auc_value` if present in the CSV rows.
  const hasPrecomputedAuc = plottedSorted.some(d => d.auc_value !== undefined && d.auc_value !== null && Number.isFinite(d.auc_value))
  if (hasPrecomputedAuc) {
    const byTrace = new Map()
    for (const d of plottedSorted) {
      if (d.auc_value === null || d.auc_value === undefined || !Number.isFinite(d.auc_value)) continue
      const timeVal = Number(d.time_hr)
      if (!byTrace.has(d.trace_id)) {
        byTrace.set(d.trace_id, {
          ...d,
          trace_id: d.trace_id,
          auc_value: d.auc_value,
          _timepoints: new Set([timeVal])
        })
      } else {
        const entry = byTrace.get(d.trace_id)
        if (!entry._timepoints) entry._timepoints = new Set()
        entry._timepoints.add(timeVal)
      }
    }
    return [...byTrace.values()].map(e => {
      const n_timepoints = e._timepoints ? [...e._timepoints].filter(t => Number.isFinite(t)).length : 1
      const out = {...e}
      out.n_timepoints = n_timepoints
      delete out._timepoints
      return out
    })
  }

  // Fallback: compute on-the-fly (original behavior)
  const byTrace = new Map()
...
cumulativeAucData = (() => {
  // Prefer precomputed `cumulative_auc` if present in the CSV rows.
  const hasPrecomputedCum = plottedSorted.some(d => d.cumulative_auc !== undefined && d.cumulative_auc !== null && Number.isFinite(d.cumulative_auc))
  if (hasPrecomputedCum) {
    // Use precomputed cumulative values directly (ensure sorted ordering)
    return plottedSorted
      .filter(d => Number.isFinite(d.cumulative_auc) && Number.isFinite(d.time_hr))
      .slice()
      .sort((a, b) => a.trace_id.localeCompare(b.trace_id) || a.time_hr - b.time_hr)
      .map(d => ({ ...d, trace_id: d.trace_id, cumulative_auc: d.cumulative_auc }))
  }
...
aucStatsContext = (() => {
  ...
  const kwStats = relevantStats.filter(r => r.comparison === "Kruskal-Wallis")
  const pairwiseStats = relevantStats.filter(r => r.comparison === "Wilcoxon")
...
aucUsesPrecomputed = plottedSorted.some(d => Number.isFinite(d.auc_value))
cumAucUsesPrecomputed = plottedSorted.some(d => Number.isFinite(d.cumulative_auc))

aucMetricDescription = aucUsesPrecomputed
  ? "blank-normalized fluorescence"
  : (dataViewSel === "Delta fluorescence"
    ? "delta fluorescence"
    : (dataViewSel === "Measurement-normalized fluorescence"
      ? "measurement-normalized fluorescence"
      : (dataViewSel === "Paper metabolism (fold-change / selected measurement)"
...
// Dots are now centered exactly on the series group
      ...
      statDesc.innerHTML = `<strong>Tests computed:</strong> Kruskal-Wallis (omnibus test across all groups), and pairwise Wilcoxon rank-sum tests with Benjamini–Hochberg p-value adjustment (blanks excluded). The replicate unit is <code>sample_id</code> when available; otherwise the analysis falls back to wells. The summary below answers whether the groups differ overall, and which group pairs differ after adjustment.`
      ...
      if (relevantStats.length === 0) {
        ...
        const kwText = kwBest
          ? `Omnibus result: ${Number(kwBest.p_adj) < 0.05 ? "groups differ" : "no evidence that groups differ"} (Kruskal-Wallis p = ${formatP(kwBest.p_value)}, BH-adjusted p = ${formatP(kwBest.p_adj)}).`
          : "Omnibus result: unavailable for this selection."

        const sigPairText = significantPairs.length > 0
          ? `Pairwise differences after BH correction: ${significantPairs.map(r => `${r.group1} vs ${r.group2} (p_adj = ${formatP(r.p_adj)})`).join(", ")}.`
          : "Pairwise differences after BH correction: none detected at the 0.05 threshold."

        summary.innerHTML = `<strong>${kwText}</strong><br>${sigPairText}<br><span style="color:#6b7280">Analysis unit: ${analysisUnitType}.</span>`
