...
timeValues = [...new Set(plottedSorted.map(d => d.time_hr))]
  .filter(Number.isFinite)
  .sort((a, b) => a - b)

timeSpacing = timeValues.length > 1
  ? Math.min(...timeValues.slice(1).map((t, i) => t - timeValues[i]).filter(d => Number.isFinite(d) && d > 0))
  : 1

jitterSpan = Math.max(0.03, Math.min(0.18, (Number.isFinite(timeSpacing) ? timeSpacing : 1) * 0.12))

jitterOffset = d => {
  const seed = `${d.trace_id}|${d.series_id}|${d.time_hr}|${d.well_id}`
  const value = hashString(seed) / 4294967295
  return (value - 0.5) * jitterSpan * 2
}
