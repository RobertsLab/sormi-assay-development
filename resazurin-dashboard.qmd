...
filtered = selectionRows.filter(d =>
  (selectedWells.length === 0 || selectedWells.includes(d.well_id)) &&
...
plottedEffective = activePlateSel === "All"
  ? (() => {
      ...
      for (const d of plotted) {
        const key = `${d.experiment_dir}|${d.well_id}|${d.time_hr}`
        if (!groups.has(key)) {
          groups.set(key, {
